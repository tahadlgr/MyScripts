...
my $src = "/usy/liberty/configureSSL/dates/${datestring}.pl";
...
foreach my $serverLine4linux (@output_array4linux) {
	...
	if ( $hostname4linux =~ m/wlp/ && grep( /^$hostname4linux$/, @whitelist )) {
		...
		my $cmd = `perl /usy/liberty/configureSSL/configureSSL.pl $hostname4linux >> /usy/liberty/configureSSL/logs/configure4All.log`;
...
my $cmd2 = `cat /usy/liberty/configureSSL/logs/configure4All.log >> /usy/liberty/configureSSL/logs/oldlogs.log`;
